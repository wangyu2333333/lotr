			...
			for (int k1 = -2; k1 <= 2; ++k1) {
				...
				block2: for (int i2 = -1; i2 <= 1; ++i2) {
					for (int k2 = -1; k2 <= 1; ++k2) {
						if (Math.abs(i2) != 0 && Math.abs(k2) != 0 || LOTRBlockSaplingBase.isSameSapling(world, i + i1 + i2, j, k + k1 + k2, block, meta)) {
							continue;
						}
						canGenerate = false;
						break block2;
			...
			for (int k1 = searchMin; k1 <= searchMax; ++k1) {
				...
				block2: for (int i2 = squareMin; i2 <= squareMax; ++i2) {
					for (int k2 = squareMin; k2 <= squareMax; ++k2) {
						int i3 = i + i1 + i2;
						int k3 = k + k1 + k2;
						if (LOTRBlockSaplingBase.isSameSapling(world, i3, j, k3, block, meta)) {
							continue;
						}
						canGenerate = false;
						break block2;
